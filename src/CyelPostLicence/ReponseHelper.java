package CyelPostLicence;

/** 
 * Helper class for : Reponse
 *  
 * @author OpenORB Compiler
 */ 
public class ReponseHelper
{
    private static final boolean HAS_OPENORB;
    static {
        boolean hasOpenORB = false;
        try {
            Thread.currentThread().getContextClassLoader().loadClass("org.openorb.CORBA.Any");
            hasOpenORB = true;
        }
        catch(ClassNotFoundException ex) {
        }
        HAS_OPENORB = hasOpenORB;
    }
    /**
     * Insert Reponse into an any
     * @param a an any
     * @param t Reponse value
     */
    public static void insert(org.omg.CORBA.Any a, CyelPostLicence.Reponse t)
    {
        a.insert_Streamable(new CyelPostLicence.ReponseHolder(t));
    }

    /**
     * Extract Reponse from an any
     * @param a an any
     * @return the extracted Reponse value
     */
    public static CyelPostLicence.Reponse extract(org.omg.CORBA.Any a)
    {
        if (!a.type().equal(type()))
            throw new org.omg.CORBA.MARSHAL();
        if (HAS_OPENORB && a instanceof org.openorb.CORBA.Any) {
            // streamable extraction. The jdk stubs incorrectly define the Any stub
            org.openorb.CORBA.Any any = (org.openorb.CORBA.Any)a;
            try {
                org.omg.CORBA.portable.Streamable s = any.extract_Streamable();
                if(s instanceof CyelPostLicence.ReponseHolder)
                    return ((CyelPostLicence.ReponseHolder)s).value;
            } catch (org.omg.CORBA.BAD_INV_ORDER ex) {
            }
            CyelPostLicence.ReponseHolder h = new CyelPostLicence.ReponseHolder(read(a.create_input_stream()));
            a.insert_Streamable(h);
            return h.value;
        }
        return read(a.create_input_stream());
    }

    //
    // Internal TypeCode value
    //
    private static org.omg.CORBA.TypeCode _tc = null;
    private static boolean _working = false;

    /**
     * Return the Reponse TypeCode
     * @return a TypeCode
     */
    public static org.omg.CORBA.TypeCode type()
    {
        if (_tc == null) {
            synchronized(org.omg.CORBA.TypeCode.class) {
                if (_tc != null)
                    return _tc;
                if (_working)
                    return org.omg.CORBA.ORB.init().create_recursive_tc(id());
                _working = true;
                org.omg.CORBA.ORB orb = org.omg.CORBA.ORB.init();
                org.omg.CORBA.StructMember []_members = new org.omg.CORBA.StructMember[2];

                _members[0] = new org.omg.CORBA.StructMember();
                _members[0].name = "etudiant";
                _members[0].type = CyelPostLicence.EtudiantHelper.type();
                _members[1] = new org.omg.CORBA.StructMember();
                _members[1].name = "ordre";
                _members[1].type = orb.get_primitive_tc(org.omg.CORBA.TCKind.tk_short);
                _tc = orb.create_struct_tc(id(),"Reponse",_members);
                _working = false;
            }
        }
        return _tc;
    }

    /**
     * Return the Reponse IDL ID
     * @return an ID
     */
    public static String id()
    {
        return _id;
    }

    private final static String _id = "IDL:CyelPostLicence/Reponse:1.0";

    /**
     * Read Reponse from a marshalled stream
     * @param istream the input stream
     * @return the readed Reponse value
     */
    public static CyelPostLicence.Reponse read(org.omg.CORBA.portable.InputStream istream)
    {
        CyelPostLicence.Reponse new_one = new CyelPostLicence.Reponse();

        new_one.etudiant = CyelPostLicence.EtudiantHelper.read(istream);
        new_one.ordre = istream.read_short();

        return new_one;
    }

    /**
     * Write Reponse into a marshalled stream
     * @param ostream the output stream
     * @param value Reponse value
     */
    public static void write(org.omg.CORBA.portable.OutputStream ostream, CyelPostLicence.Reponse value)
    {
        CyelPostLicence.EtudiantHelper.write(ostream,value.etudiant);
        ostream.write_short(value.ordre);
    }

}
